...
export default function Sidebar({ 
  ...
  const handleNavigate = (view: View, filters?: { companyId?: string | number, projectId?: string | number }) => {
    if (filters) {
  ...
  return (
    <>
      ...
      <nav className={`fixed left-0 top-0 h-screen flex flex-col py-4 border-r border-[#0F172A05] bg-[#FAFAFB] lg:rounded-r-[24px] shadow-[1px_0_10px_rgba(0,0,0,0.01)] z-40 transition-all duration-300 ${isCollapsed ? '-translate-x-full lg:translate-x-0 lg:w-[80px] px-2' : 'translate-x-0 w-[280px] px-3'}`}>
        ...
        <div className={`flex items-center mb-6 px-3 ${isCollapsed ? 'lg:justify-center' : 'gap-3'}`}>
            ...
            <AnimatePresence>
              {empresasExpanded && !isCollapsed && (
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="flex flex-col gap-0.5">
                  {companies.length === 0 && (
                    <div className="text-[11px] text-[#94A3B8] px-8 py-2 font-medium">Nenhuma empresa.</div>
                  )}
        ...
        <div className="mt-auto pt-3 border-t border-[#0F172A05] px-2">
          {!isCollapsed ? (
            <div className="flex items-center justify-between p-2 rounded-xl bg-white border border-[#0F172A05] shadow-sm cursor-pointer hover:bg-gray-50 transition-colors">
    ...
    <div className="flex flex-col">
      ...
      >
        <div className="flex items-center gap-2 overflow-hidden flex-1">
          ...
          <span className="text-[13px] font-medium truncate">{company.name}</span>
        </div>
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
...
function SubNode({ icon: Icon, label, count, onClick, active, items, itemType, handleNavigate, companyId }: any) {
  ...
  return (
    <div className="flex flex-col">
      <div 
        className={`group flex items-center justify-between py-1.5 pr-2 pl-[28px] rounded-lg cursor-pointer transition-all ${active ? 'text-[#111111] font-bold bg-[#111111]/5' : 'text-[#64748B] hover:bg-[#F1F5F9]'}`}
        onClick={(e) => {
          ...
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="flex flex-col gap-0.5">
            {items.map((item: any) => (
              <LeafNode 
                ...
                active={false} // Needs deeper comparison for item level active
              />
            ))}
          </motion.div>
        )}
...
function LeafNode({ icon: Icon, label, indent = 0, onClick, active }: any) {
  return (
    <div 
      className={`group flex items-center justify-between py-1 pr-2 rounded-lg cursor-pointer transition-all ${active ? 'text-[#111111] font-bold bg-[#111111]/5' : 'text-[#64748B] hover:bg-[#F1F5F9]'}`}
      style={{ paddingLeft: `${48 + indent * 16}px` }}
      onClick={onClick}
    >
      <div className="flex items-center gap-2 overflow-hidden flex-1">
        <Icon size={12} className="flex-shrink-0 text-[#94A3B8]" />
