...
export default function ProjectContent({ project, onSave, onClose }: ProjectContentProps) {
...
const prevProjectIdRef = useRef<number | null>(null);
  ...
  // Initialize product data when project changes
  useEffect(() => {
    if (editedProject) {
      setProductData({
...
useEffect(() => {
  if (!project) return;
  if (project.id === prevProjectIdRef.current) return;
...
prevProjectIdRef.current = project.id;
  ...
  return (
    <>
    <div className="relative w-full bg-[#FFFFFF] flex flex-col overflow-hidden">
