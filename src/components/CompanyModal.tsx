            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-[10px] font-medium text-[#64748B] mt-0.5">
                  Registre uma nova organização no ecossistema
                </p>
              </div>
            </div>
            <button 
            ...
            <div className="flex px-8 gap-5 overflow-x-auto bg-[#111111] border-t border-white/5 scrollbar-none">
              <button 
                ...
                className={`py-4 px-1 border-b-2 text-xs font-bold transition-all ${
                  activeModalTab === 'cadastro' ? 'border-white text-white' : 'border-transparent text-white/50 hover:text-white hover:border-white/20'
                }`}
              >
                Dados Cadastrais
              </button>
              <button 
                onClick={() => setActiveModalTab('identidade_visual')}
                className={`py-4 px-1 border-b-2 text-xs font-bold transition-all ${
                  activeModalTab === 'identidade_visual' ? 'border-white text-white' : 'border-transparent text-white/50 hover:text-white hover:border-white/20'
                }`}
              >
                Identidade Visual
              </button>
              <button 
                ...
                className={`py-4 px-1 border-b-2 text-xs font-bold transition-all ${
                  activeModalTab === 'visao_360' ? 'border-white text-white' : 'border-transparent text-white/50 hover:text-white hover:border-white/20'
                }`}
              >
                Visão 360°
              </button>
        ...
        {company && activeModalTab === 'visao_360' ? (
          ...
          <div className="h-[60vh] overflow-y-auto flex flex-col justify-between">
            <VisualIdentityTab 
              ...
              logoUrl={formData.logoUrl}
              coverUrl={formData.coverUrl}
              onChangeLogo={(url) => setFormData(prev => ({ ...prev, logoUrl: url }))}
              onChangeCover={(url) => setFormData(prev => ({ ...prev, coverUrl: url }))}
            />
            
            {/* Action buttons inside identity tab */}
            <div className="p-6 flex items-center justify-end gap-3 border-t border-[#0F172A05]">
              ...
              <button 
                ...
                disabled={loading}
                className="px-5 py-2 text-xs font-bold text-white bg-[#111111] hover:bg-[#222222] rounded-xl shadow-sm transition-all cursor-pointer flex items-center gap-1.5"
              >
                {loading ? 'Salvando...' : (
                  <>
