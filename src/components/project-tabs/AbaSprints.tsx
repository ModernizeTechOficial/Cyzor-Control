...
import { Play, Calendar, CheckSquare, BarChart, Plus, ChevronRight, Zap, Target, List, LayoutGrid, Clock, Trash2 } from 'lucide-react';
...
export default function AbaSprints({ project, onUpdateProject }: AbaSprintsProps) {
  ...
  const timelineItems = useMemo(() => {
    return sprintTasks.map(task => {
      ...
      return {
        ...
        endDate: endDateStr,
        status: task.column,
        statusLabel: getColLabel(task.column),
  ...
  const handleUpdateItemDates = async (taskId: number, newStartDate: string, newEndDate: string) => {
    ...
    try {
        ...
        if (res.ok) {
            const updatedTasks = (project.tasks || []).map(t => t.id === taskId 
        ...
        console.error("Error updating sprint task dates in timeline:", e);
    }
  };
      ...
      {selectedSprint ? (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          ...
          <div className="lg:col-span-2 flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-4">
              <div className="flex justify-between items-center border-b border-[#0F172A0F] pb-2">
                      ...
                      <tbody className="divide-y divide-[#0F172A05]">
                        {sprintTasks.map(task => (
                          ...
                          >
                            ...
                            <td className="py-3 px-3">
                              <span className="capitalize text-slate-600 bg-slate-50 border border-slate-100 px-1.5 py-0.5 rounded text-[9px] font-bold">
                                {getColLabel(task.column)}
                              </span>
                            </td>
