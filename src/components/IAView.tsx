...
export default function IAView() {
  ...
  useEffect(() => {
    const fetchInsightsAndStats = async () => {
      try {
        ...
        promises.push(
          fetch('/api/ai/memory-stats', { headers: { 'Authorization': `Bearer ${token}` } })
            .then(res => res.json())
  ...
  return (
    <div className="flex gap-8 h-[calc(100vh-100px)]">
      ...
      <div className="flex-1 flex flex-col bg-[#FFFFFF] rounded-[30px] border border-[#0F172A0F] shadow-[0_20px_60px_rgba(0,0,0,0.02)] overflow-hidden">
        
        {/* Header */}
        <div className="px-8 py-6 border-b border-[#0F172A0F] bg-[#FAFAFA] flex items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-[16px] bg-[#111111] flex items-center justify-center shadow-md">
              <BotMessageSquare size={24} className="text-white" />
            </div>
            <div>
              <h1 className="text-2xl font-display font-bold text-[#111111] tracking-tight">Olimpo AI</h1>
        ...
        <div className="flex-1 overflow-y-auto p-8 flex flex-col gap-6 custom-scrollbar">
          {messages.map((msg, idx) => (
            <div key={idx} className={`flex gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
              <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${msg.role === 'user' ? 'bg-[#FAFAFA] border border-[#0F172A0F] text-[#111111] font-bold text-sm' : 'bg-[#111111] text-white shadow-md'}`}>
                {msg.role === 'user' ? 'U' : <Sparkles size={18} />}
              </div>
              <div className={`max-w-[70%] p-4 text-sm leading-relaxed ${
                msg.role === 'user' 
                  ? 'bg-[#111111] text-white rounded-[20px] rounded-tr-none shadow-md' 
                  : 'bg-[#FAFAFA] border border-[#0F172A0F] text-[#475569] rounded-[20px] rounded-tl-none shadow-sm'
              }`}>
                {msg.text}
              </div>
            </div>
          ...
          {messages.length === 1 && (
            <div className="mt-8 flex flex-col gap-3">
              <h3 className="text-[11px] font-bold uppercase text-[#64748B] tracking-widest pl-2">Sugestões de perguntas</h3>
              <div className="flex flex-wrap gap-2">
                {SUGGESTIONS.map((suggestion, idx) => (
                  <button 
                    key={idx} 
                    onClick={() => handleSend(suggestion)}
                    className="px-4 py-2 bg-[#FFFFFF] border border-[#0F172A0F] rounded-[16px] text-xs font-semibold text-[#111111] hover:bg-[#FAFAFA] hover:shadow-sm transition-all"
                  >
                    {suggestion}
                  </button>
                ))}
        ...
        {/* Input Area */}
        <div className="p-6 border-t border-[#0F172A0F] bg-[#FFFFFF] flex-shrink-0">
          <div className="relative group flex items-center">
            <input 
              ...
              placeholder="Pergunte sobre seus projetos, documentação ou dados financeiros..."
              className="w-full bg-[#FAFAFA] border border-[#0F172A0F] rounded-[20px] py-4 pl-6 pr-16 outline-none focus:border-[#111111]/30 hover:border-[#0F172A0F]-dark transition-all text-[#111111] font-medium placeholder:text-[#64748B]/50 shadow-[0_2px_8px_rgba(0,0,0,0.01)]"
            />
            <button 
              onClick={() => handleSend(query)}
              disabled={!query.trim()}
              className="absolute right-2 top-1/2 -translate-y-1/2 w-10 h-10 rounded-[14px] bg-[#111111] text-[#FFFFFF] flex items-center justify-center hover:bg-black transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
      ...
      {/* Painel Lateral */}
      <div className="w-[380px] flex-shrink-0 flex flex-col gap-6 overflow-y-auto custom-scrollbar pb-6 pr-2 h-full">
