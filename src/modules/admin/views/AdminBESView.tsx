...
import { BES_STAGES, ACTIONS } from '../../../utils/besCalculator';
import StandardHeader from '../../../components/layout/StandardHeader';

export default function AdminBESView() {
  const { fetchWithAuth } = useAuth();
  const [workspaces, setWorkspaces] = useState<any[]>([]);
  const [besConfig, setBesConfig] = useState<{actions: any} | null>(null);
  ...
  const fetchWorkspaces = async () => {
    ...
    try {
      ...
      if (resConfig.ok) {
        ...
        setBesConfig(data.actions ? data : { actions: ACTIONS });
      } else {
        setBesConfig({ actions: ACTIONS });
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <div id="admin-bes-module" className="w-full mx-auto pb-12 flex flex-col gap-10 animate-in fade-in duration-500 relative px-4 sm:px-6 lg:px-10">
      ...
      <StandardHeader 
        title="Configuração Global BES"
        subtitle="Ajuste os pesos dos eventos que compõem o algoritmo do Business Event Score."
        actions={[
          {
            label: 'Recarregar',
            icon: RotateCw,
      ...
      <div className="bg-white border rounded-[24px] shadow-sm overflow-hidden p-2">
        <div className="p-6 pb-2 border-b border-slate-100 flex flex-col gap-1">
          <h3 className="font-display font-black text-sm text-zinc-900">Monitoramento de BES (Leitura)</h3>
        ...
        <table className="w-full text-xs">
          <thead className="bg-zinc-50 border-b">
            <tr>
              ...
              <th className="text-right px-6 py-4 font-display font-black text-zinc-500 uppercase">Score Atual (BES)</th>
              <th className="text-right px-6 py-4 font-display font-black text-zinc-500 uppercase">Nível Derivado</th>
            </tr>
          </thead>
          <tbody>
            {workspaces.map((ws) => {
              const bes = ws.settings?.besScore || 0;
              const currentStage = BES_STAGES.find(s => bes >= s.min && bes <= s.max) || BES_STAGES[0];
              return (
                <tr key={ws.id} className="border-b hover:bg-zinc-50/50">
                  <td className="px-6 py-4 font-display font-bold">{ws.name}</td>
                  <td className="px-6 py-4 text-right font-mono font-bold text-blue-600">{bes.toLocaleString()}</td>
